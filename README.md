...
- 订单状态驱动：AI 只生成 `待确认`，`已确认` 和 `已发货` 完全信任飞书人工维护结果。
...
- 库存预留：待确认订单写入 AI排单_库存预留表，后续排单会扣减有效预留。
...
AI 排单总表通过 `订单状态` 驱动后续逻辑：

- `待确认`：AI 新生成或重新生成的建议状态。
- `已确认`：人工确认后的状态，会参与锁单库存占用。
- `已发货`：人工维护的最终状态，系统信任并保留，不再参与锁单占用。

AI 只写入 `待确认`。`已确认` 和 `已发货` 必须由人工在飞书中维护，系统不会覆盖这些人工状态。

### 库存预留

AI排单_库存预留表用于记录待确认订单占用的库存，字段包括：
...
```text
库存可用量 = 库存快照数量 - 已确认订单 SKU 需求总和 - 有效预留数量
```
...
TABLE_ID_RESERVATION=your_inventory_reservation_table_id
```
